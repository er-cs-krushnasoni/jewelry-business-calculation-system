    ...
    return (
      <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <p className="font-semibold text-yellow-800">{t('rates.manager.accessRestricted')}</p>
            <p className="text-yellow-700">{t('rates.manager.accessRestrictedMessage')}</p>
          </div>
        </div>
      </div>
    );
    ...
    return (
      <div className="bg-white rounded-lg shadow p-6">
        <div className="flex items-center justify-center space-x-3">
          ...
          <span className="text-gray-600">{t('rates.manager.loading')}</span>
        </div>
      </div>
    );
  }
    ...
    <div className="bg-white rounded-lg shadow">
      {showTitle && (
        <div className="px-6 py-4 border-b border-gray-200">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-xl font-semibold text-gray-800">{t('rates.manager.title')}</h2>
              <p className="text-gray-600 mt-1">{t('rates.manager.subtitle')}</p>
            </div>
            ...
            <div className="flex items-center space-x-2">
              {isConnected ? (
                <div className="flex items-center space-x-2 text-green-600">
                  <Wifi size={16} />
                  <span className="text-sm font-medium">{t('rates.manager.realTimeActive')}</span>
                </div>
              ) : (
                <div className="flex items-center space-x-2 text-gray-500">
                  <WifiOff size={16} />
                  <span className="text-sm font-medium">{t('rates.manager.offlineMode')}</span>
                </div>
              )}
      ...
      <div className="p-6">
        ...
        {currentRates && updateInfo && (
          <div className="mb-6 bg-blue-50 border border-blue-200 rounded-lg p-4">
            <div className="flex items-center space-x-2 mb-2">
              ...
              <span className="font-semibold text-blue-800">{t('rates.manager.currentRates')}</span>
              {realTimeUpdate && (
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div>
                <span className="text-gray-600">{t('rates.display.gold')} </span>
                <span className="font-medium">
                  {t('rates.manager.gold.buy', { rate: currentRates.goldBuy })}, {t('rates.manager.gold.sell', { rate: currentRates.goldSell })}
                </span>
              </div>
              <div>
                <span className="text-gray-600">{t('rates.display.silver')} </span>
                <span className="font-medium">
                  {t('rates.manager.silver.buy', { rate: currentRates.silverBuy })}, {t('rates.manager.silver.sell', { rate: currentRates.silverSell })}
                </span>
              </div>
            </div>
            <div className="flex items-center space-x-4 mt-3 text-xs text-gray-600">
              <div className="flex items-center space-x-1">
                <User size={12} />
                <span>{t('rates.manager.updatedBy')} {updateInfo.updatedBy} ({updateInfo.role})</span>
              </div>
              <div className="flex items-center space-x-1">
                <Clock size={12} />
              ...
              {updateInfo.isToday && (
                <span className="bg-green-100 text-green-800 px-2 py-0.5 rounded">{t('rates.display.today')}</span>
              )}
            </div>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          {/* Gold Rates */}
          <div>
            <h3 className="text-lg font-medium text-gray-800 mb-3">{t('rates.manager.goldRatesTitle')}</h3>
          ...
          {/* Silver Rates */}
          <div>
            <h3 className="text-lg font-medium text-gray-800 mb-3">{t('rates.manager.silverRatesTitle')}</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Input
          ...
          <div className="flex justify-end">
            ...
            >
              ...
              <span>{saving ? t('rates.manager.updating') : t('rates.manager.updateRates')}</span>
            </Button>
          </div>
        </form>
        ...
        <div className="mt-8 bg-gray-50 rounded-lg p-4">
          <h4 className="font-medium text-gray-800 mb-2">{t('rates.manager.importantNotes')}</h4>
          <ul className="text-sm text-gray-600 space-y-1">
            <li>• {t('rates.manager.note1')}</li>
            <li>• {t('rates.manager.note2')}</li>
            <li>• {t('rates.manager.note3')}</li>
            <li>• {isConnected ? t('rates.manager.note4Connected') : t('rates.manager.note4Disconnected')}</li>
            <li>• {t('rates.manager.note5')}</li>
            {!isConnected && (
              <li className="text-yellow-600 font-medium">• {t('rates.manager.note6Offline')}</li>
            )}
          </ul>
        </div>
